...
SOUND_EFFECT_DISCHARGE="button_synth_negative_02.wav"
SOUND_EFFECT_CHARGE="sound_buttons_button_synth_positive_01.wav"
...
upower --monitor-detail | while read -r LINE; do
  ...
  state:*discharging)
    if [ "$STATE" != DISCHARGING ]; then
      ...
      play --volume "$VOLUME" "$SOUND_EFFECT_FOLDER/$SOUND_EFFECT_DISCHARGE"
      STATE=DISCHARGING
    fi
    ;;
  state:*pending-charge | state:*charging)
    if [ "$STATE" != CHARGING ]; then
      ...
      play --volume "$VOLUME" "$SOUND_EFFECT_FOLDER/$SOUND_EFFECT_CHARGE"
      STATE=CHARGING
    fi
    ;;
  *) # do nothing
